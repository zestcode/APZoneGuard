...
struct scan_result_item {
	char ssid[MAX_SSID_LEN + 1];
	int rssi;
	bool seen;
};

static struct scan_result_item Scan_Result[5];
static const char * const local_aps[] = { "AP_0", "AP_1", "AP_3" };
...
static int local_ap_index(const uint8_t *ssid, uint8_t ssid_len)
{
	size_t i;

	for (i = 0; i < ARRAY_SIZE(local_aps); i++) {
		size_t len = strlen(local_aps[i]);

		if (ssid_len == len && memcmp(ssid, local_aps[i], len) == 0) {
			return (int)i;
		}
	}
...
static int send_payload(const char *payload, size_t payload_len)
{
	int sock;
	int ret;
	ssize_t sent;
	...
	if (sock < 0) {
		LOG_ERR("TCP socket create failed: %d", errno);
		return -errno;
	}
	...
	ret = inet_pton(AF_INET, TCP_SERVER_IP, &addr.sin_addr);
	if (ret != 1) {
		LOG_ERR("Invalid TCP server IP: %s", TCP_SERVER_IP);
		close(sock);
		return -EINVAL;
	}
	...
	sent = send(sock, payload, payload_len -1 , 0);
	if (sent < 0) {
	...
	LOG_INF("TCP payload sent: %d bytes", (int)sent);
	close(sock);
	return 0;
}
...
static void handle_wifi_scan_result(struct net_mgmt_event_callback *cb)
{
	...
	if (!result || result->ssid_length == 0) {
		return;
	}

	idx = local_ap_index(result->ssid, result->ssid_length);
	if (idx < 0) {
		return;
	}

	if (idx >= (int)ARRAY_SIZE(Scan_Result)) {
		return;
	}

	memset(Scan_Result[idx].ssid, 0, sizeof(Scan_Result[idx].ssid));
	memcpy(Scan_Result[idx].ssid, result->ssid,
	       MIN(result->ssid_length, MAX_SSID_LEN));
	Scan_Result[idx].rssi = result->rssi;
	Scan_Result[idx].seen = true;

	LOG_INF("Matched AP: SSID=%s, RSSI=%d", Scan_Result[idx].ssid, Scan_Result[idx].rssi);
}

static char my_payload[1024];
...
static void handle_wifi_scan_done(struct net_mgmt_event_callback *cb)
{
...

	{
		...
		offset += snprintf(my_payload + offset, sizeof(my_payload) - offset,
				   "{\"tag_id\":\"tag_01\",\"device_id\":\"nrf7002_001\",\"measurements\":[");

		for (i = 0; i < (int)ARRAY_SIZE(Scan_Result); i++) {
			if (!Scan_Result[i].seen) {
				continue;
			}
			offset += snprintf(my_payload + offset, sizeof(my_payload) - offset,
					   "%s{\"ap_id\":\"%s\",\"rssi\":%d}",
		...
		LOG_INF("Payload: %s", my_payload);

		if (!first && context.connected) {
			send_payload(my_payload, strlen(my_payload));
		}
	}
...
static int wifi_scan_request(void)
{
	struct net_if *iface = net_if_get_first_wifi();
	...
	if(context.connected == false) {
		LOG_WRN("Not connected, skipping scan");
		return -EALREADY;
	}
...
static void wifi_scan_thread(void)
{
	while (1) {
#ifdef CONFIG_WIFI_READY_LIB
...
K_THREAD_DEFINE(scan_thread_id, SCAN_THREAD_STACK_SIZE, wifi_scan_thread,
